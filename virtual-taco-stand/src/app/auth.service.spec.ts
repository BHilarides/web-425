...
describe('AuthService', () => {
  ...
  it('should set cookie and authState to true on successful signin', () => {
    ...
    expect(service.gerAuthState().subscribe(state => expect(state).toBeTrue()));
    expect(cookieSerciceSpy.set.calls.count()).toBe(1);
  });
  ...
  it('should not set cookie and authState to true on unsuccessful signin', () => {
    ...
    expect(service.getAuthState().subsctibe(state => expect(state).toBeFalse()));
